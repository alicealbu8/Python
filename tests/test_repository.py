...
class TestRepository(TestCase):
    ...
    def setUp(self):
        self.plane_repo = PlaneRepository([
            Plane(1, "Airline1", 180, "Rome",[Passenger("a", "a", "NC12456"), Passenger("a", "b", "AL12390"), Passenger("a", "c", "OK90897")]),
            Plane(2, "Airline2", 80, "Paris",[Passenger("b", "a", "LJ12456"), Passenger("b", "b", "GH12390")]),
            Plane(3, "Airline3", 200, "Budapest", [Passenger("c", "c", "ER12456"), Passenger("c", "b", "PY12390"), Passenger("c", "a", "TR5690")]),
            Plane(4, "Airline4", 100, "Vienna", [Passenger("d", "a", "WQ12456"), Passenger("d", "b", "TK12390"), Passenger("d", "c", "QS8790")])
                                            ])
    def test_get_planes(self):
        '''
        Test function for get planes
        :return:
        '''
    ...
    def test_add_plane(self):
        ...
        :return:
        '''
        self.plane_repo.add_plane(5, "Airline5", 150, "Prague", [Passenger("e", "a", "AD12456"), Passenger("e", "b", "AF12390"), Passenger("e", "c", "QA8790")])
        list_planes = self.plane_repo.get_planes()
    ...
    def test_update_by_number(self):
        ...
        :return:
        '''
        self.plane_repo.update_by_number(1, "AirlineA", 300, "Bucharest", [Passenger("e", "a", "AD12456"), Passenger("e", "b", "AF12390"), Passenger("e", "c", "QA8790")])
        list_planes = self.plane_repo.get_planes()
    ...
    def test_delete_by_number(self):
        ...
        :return:
        '''
        self.plane_repo.delete_by_number(1)
        list_planes = self.plane_repo.get_planes()
    ...
    def test_sort_by_last_name(self):
        ...
        :return:
        '''
        self.plane_repo.sort_by_last_name(1)
        list_planes = self.plane_repo.get_planes()
        ...
        self.plane_repo.sort_by_last_name(3)
    ...
    def test_sort_by_number_of_passengers(self):
        ...
        :return:
        '''
        self.plane_repo.sort_by_nr_of_passengers()
        list_planes = self.plane_repo.get_planes()
    ...
    def test_sort_planes_acc_to_concatenation(self):
        ...
        :return:
        '''
        self.plane_repo.sort_planes_acc_to_concatenation()
        list_planes = self.plane_repo.get_planes()
    ...
    def test_sort_by_substring(self):
        ...
        '''
        self.plane_repo = PlaneRepository([
            Plane(1, "Airline1", 180, "Rome",[Passenger("aaa", "a", "NC12456"), Passenger("aa", "b", "AL12390"), Passenger("abc", "c", "OK90897")]),
        ...
        self.assertEqual(list_planes[0].getNumber(), 4) #we can check only the number of the plane because it is unique
        self.assertEqual(list_planes[1].getNumber(), 3)
    ...
    def test_identify_planes_passport(self):
        ...
        '''
        self.plane_repo = PlaneRepository([
            Plane(1, "Airline1", 180, "Rome", [Passenger("a", "a", "NC12456"), Passenger("a", "b", "NC12390"), Passenger("a", "c", "NC10897")]),
            Plane(2, "Airline2", 80, "Paris", [Passenger("b", "a", "LJ12456"), Passenger("b", "b", "GH12390")]),
            Plane(3, "Airline3", 200, "Budapest", [Passenger("c", "c", "ER12456"), Passenger("c", "b", "PY12390"), Passenger("c", "a", "TR5690")]),
            Plane(4, "Airline4", 100, "Vienna", [Passenger("d", "a", "WQ12456"), Passenger("d", "b", "TK12390"), Passenger("d", "c", "QS8790")])
        ])
        ...
        self.plane_repo.update_by_number(2, "Airline2", 80, "Paris", [Passenger("b", "a", "LJ12456"), Passenger("b", "b", "LJ12390")])
        list = self.plane_repo.identify_planes_passport()

        self.assertEqual(list[0].getNumber(), 1)
        self.assertEqual(list[1].getNumber(), 2)

        self.plane_repo.update_by_number(1, "Airline1", 180, "Rome", [Passenger("a", "a", "C12456"), Passenger("a", "b", "NC12390"), Passenger("a", "c", "NC10897")])
        list = self.plane_repo.identify_planes_passport()
    ...
    def test_identify_passengers(self):
        ...
        list = self.plane_repo.identify_passengers(1, "a")  #we can check only the passport of the passenger because it is unique
        self.assertEqual(list[0].getPassport(), "NC12456")
        self.assertEqual(list[1].getPassport(), "AL12390")
        self.assertEqual(list[2].getPassport(), "OK90897")

        list = self.plane_repo.identify_passengers(3, "b")
        self.assertEqual(list[0].getPassport(), "PY12390")

    def test_identify_planes_with_passengers(self):
        '''

        :return:
        '''
        list = self.plane_repo.identify_plane_with_passenger("a", "a")
        self.assertEqual(list[0].getNumber(), 1)

    def test_construct_solutionPlanes(self):
        '''

        :return:
        '''
        MyPlanes = PlaneRepository([Plane(1, "w", 180, "Rome",
                                          [Passenger("a", "a", "12456"), Passenger("b", "b", "12390"),
                                           Passenger("c", "b", "90")]),
                                    Plane(2, "w", 90, "Prague",
                                          [Passenger("b", "mao", "125678"), Passenger("ionela", "mara", "12390"),
                                           Passenger("ionel", "marin", "12")]),
                                    Plane(3, "a", 90, "Prague",
                                          [Passenger("b", "mao", "125678"), Passenger("ionela", "mara", "12390"),
                                           Passenger("ionel", "marin", "12")])
                                    ])

        for s in MyPlanes.groupPlanes(2):
            MyPlanes.constructSolutionPlanes(s)
            self.assertEqual(MyPlanes.constructSolutionPlanes(s)[0].getNumber(), 2)
            break
        for s in MyPlanes.groupPlanes(2):
            MyPlanes.constructSolutionPlanes(s)
            self.assertEqual(MyPlanes.constructSolutionPlanes(s)[1].getNumber(), 3)
            break
        for s in MyPlanes.groupPlanes(3):
            MyPlanes.constructSolutionPlanes(s)
            self.assertEqual(MyPlanes.constructSolutionPlanes(s)[0].getNumber(), None)
    ...
    def test_construct_solution(self):
        ...
        for s in MyPlanes.groupPersons(2, 1):
            MyPlanes.constructSolution(s, 1)
            self.assertEqual(MyPlanes.constructSolution(s, 1)[0].getPassport(), "12456")
            break
        for s in MyPlanes.groupPersons(2, 2):
            MyPlanes.constructSolution(s, 2)
            self.assertEqual(MyPlanes.constructSolution(s, 2)[0].getPassport(), "125678")
            break
